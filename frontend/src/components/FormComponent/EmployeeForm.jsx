...
const EmployeeForm = ({
    ...
    const handleSelectChange = (value, name) => {
        ...
        if (name === "unit_id") {
            setAvailableUnits(
                availableUnits.filter((unit) => unit.value !== value),
            );
        }
        if (name === "gender") {
            setAvailableGenders(
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50 gap-5">
            <Card color="white" shadow={5} className="w-full max-w-md p-6 ">
                ...
                <form onSubmit={handleSubmit} className="flex flex-col gap-5">
                    ...
                    <Input
                        label="Employee Name"
                        placeholder={formData.name || "Enter employee name"}
                        value={formData.name}
                    ...
                    <Input
                        label="Role"
                        value={formData.role} // Corrected from FormData.role
                        onChange={handleInputChange}
                        name="role"
                        required
                    />
                    ...
                    <Select
                        ...
                        onChange={(value) =>
                            handleSelectChange(value, "unit_id")
                        }
                        required
                    >
